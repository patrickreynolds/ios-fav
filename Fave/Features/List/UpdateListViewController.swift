...
protocol UpdateListViewControllerDelegate {
    func didUpdateList(list: List)
}

class UpdateListViewController: FaveVC {

    let list: List

    var delegate: UpdateListViewControllerDelegate?
    ...
    var updateListEnabled: Bool = true {
        didSet {
    ...
    private lazy var contentView: UIView = {
        ...
        commentTextView.text = list.description
        ...
        commentTextViewPlaceholder.isHidden = true
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let titleViewLabel = Label(text: "Update list", font: FaveFont(style: .h5, weight: .bold), textColor: FaveColors.Black80, textAlignment: .center, numberOfLines: 1)
    ...
    @objc func createListButtonTapped(sender: UIButton!) {
        ...
        isLoading = true
        dependencyGraph.faveService.updateList(listId: list.id, title: title, description: description, isPublic: isPublic) { list, error in
            self.isLoading = false

            guard let unwrappedList = list else {

                let alertController = UIAlertController(title: "Error", message: "Oops, something went wrong. Try updating your list again.", preferredStyle: .alert)
            ...
            self.delegate?.didUpdateList(list: unwrappedList)
...
extension UpdateListViewController: UITextViewDelegate {
